...
const RecipeDetails = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 bg-white rounded shadow-md my-8">
      ...
      <div className="flex items-center mb-4">
        ...
        <span className="text-gray-800 font-semibold">
          {recipe.createdByDetails.username}
        </span>
      </div>
      ...
      {recipe.tags && recipe.tags.length > 0 && (
        <div className="flex flex-wrap mt-4">
          {recipe.tags.map((tag, index) => (
            <span
              key={index}
              className="bg-orange-200 text-orange-600 px-2 py-1 rounded-full text-sm mr-2 mb-2"
            >
              {tag}
            </span>
          ))}
        ...
        <div className="mt-4">
          ...
          <ul className="list-disc list-inside">
            {recipe.comments.map((commentId) => {
              const comment = fakeData.comments.find((c) => c.id === commentId); // Find the full comment object
              return (
                <li key={commentId} className="flex items-start mb-2">
                  <img
