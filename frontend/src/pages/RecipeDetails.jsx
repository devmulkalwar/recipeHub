...
const RecipeDetails = () => {
  ...
  const { user } = useAuth();
  const { getRecipeById, likeRecipe, unlikeRecipe, saveRecipe, loading } = useRecipe();
  ...
  const handleLike = async () => {
    ...
    try {
      const response = await (isLiked ? unlikeRecipe(id) : likeRecipe(id));
      setRecipe(prev => ({
        ...prev,
        likes: isLiked 
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await saveRecipe(id);
      setIsSaved(!isSaved);
      toast.success(response.message);
    } catch (error) {
      toast.error(error.message);
      ...
      <div className="bg-white rounded-lg shadow-lg overflow-hidden">
        <div className="p-6">
          ...
          <div className="flex gap-4 mb-6">
            ...
            >
              {isLiked ? <AiFillLike className="text-orange-500" /> : <AiOutlineLike />}
